...
function moduloCrud(string $titulo, string $seccion, string $controlador, array $columnas, string $campos, string $ayuda = '', array $opciones = []): void
{
    paginaPrivada($titulo, $seccion, static function () use ($titulo, $seccion, $controlador, $columnas, $campos, $ayuda, $opciones): void {
        ...
        $word = !empty($opciones['word']) && $permisos->autorizar($seccion, 'ver');
        $acciones = $editar || $anular || $word;
        $config = ['id' => (string)($opciones['id'] ?? ''), 'crear' => $crear, 'editar' => $editar, 'anular' => $anular, 'word' => $word, 'voucher' => !empty($opciones['voucher']) && $permisos->autorizar($seccion, 'ver'), 'cards' => !empty($opciones['cards']), 'metrics' => $opciones['metricas'] ?? []];
        ?>
